...
public_key = "\$(./tuic-server reality gen --public)"
private_key = "\$(./tuic-server reality gen --private)"
[restful]
...
generate_link() {
    ...
    # 节点输出链接
    cat > "$LINK_TXT" <<EOF
tuic://${TUIC_UUID}:${TUIC_PASSWORD}@${ip}:${TUIC_PORT}?congestion_control=bbr&alpn=h3&allowInsecure=0&sni=www.microsoft.com&udp_relay_mode=native&disable_sni=0&reduce_rtt=1&max_udp_relay_packet_size=8192&reality=1&short_id=$(openssl rand -hex 2)#TUIC-${ip}
EOF
